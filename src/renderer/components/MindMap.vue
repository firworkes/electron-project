...
export default {
  ...
  methods: {
    ...
    click(d, i, n) {
      ...
      if (edit) {
        // 正在编辑
      } else if (clickedNode.isSameNode(sele)) {
        // 进入编辑状态
        this.editNode(clickedNode);
      } else {
        // 选中
    ...
    cancelDrag() {
      this.mindmap_g.selectAll("g.node").call(
        d3
          .drag()
          .on("drag", null)
          .on("end", null)
    ...
    dragged(a, i, n) {
      // 拖拽中【待完善】
      const {
        draggedNodeChildrenRenew,
        draggedNodeRenew,
        mindmap_g,
        xSpacing
      } = this;
      const draggedNode = n[i];
      const draggedNodeRect = draggedNode.getElementsByTagName(
        "foreignObject"
      )[0];
      // 选中
      const sele = document.getElementById("selectedNode");
    ...
    dragended(d, i, n) {
      ...
      if (draggedParentNode.isEqualNode(this.$refs.content)) {
        // 拖拽的是根节点时复原
        dragback(subject, draggedNode);
        return;
      }
    ...
    gBtnTransform(d) {
      return `translate(${d.data.size[1] + 8 - this.xSpacing},${d.data.size[0] /
        2 -
        12})`;
    },
    pathId(d) {
    ...
    appendNode(enter) {
      ...
      const foreignDiv = foreign
        ...
        .text(d => d.data.name);
      foreignDiv.on("blur", updateNodeName).on("keydown", divKeyDown);
    ...
    updateNode(update) {
      ...
      update.each((d, i, n) => {
        ...
        const foreign = node
          .selectAll("foreignObject")
          .filter(
            (d, i, n) => d.data.id !== "0" && n[i].parentNode === node.node()
          )
          .attr("y", foreignY)
  ...
  div.button {
    ...
    button.icon {
      ...
      i {
        width: 24px;
        height: 24px;
        filter: invert(25%) sepia(40%) saturate(5050%) hue-rotate(227deg)
          brightness(78%) contrast(74%);
        // &.gps {
